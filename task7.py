...
with open('task7.txt', 'r', encoding='utf-8') as file:
    for line in file:
        ...
        for el in line.split():
            if el.isnumeric() and finance == 0:
                finance = int(el)
            elif el.isnumeric():
                finance -= int(el)
                if finance > 0:
                    profit_dict[line.split()[0]] = finance
                    finance = 0
                else:
                    losses_dict[line.split()[0]] = finance
                    finance = 0
result = round(sum(profit_dict.values()) / (count - len(losses_dict)), 2)
result_list = [profit_dict, {'Средняя прибыль компаний составила': result}]
with open('result_task7.json', 'w', encoding='utf-8') as res:
